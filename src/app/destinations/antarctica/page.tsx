...
export default function AntarcticaPage({ params }: { params: { slug: string } }) {
  return (
    <div className="bg-background text-foreground">
      <div className="container mx-auto px-4 py-16 sm:px-6 lg:px-8">
        <div className="mb-12 text-center">
          <h1 className="text-4xl font-bold sm:text-5xl md:text-6xl">Antarctica: The Last Frontier</h1>
          <p className="mx-auto mt-4 max-w-3xl text-lg text-muted-foreground">Journey to the world&apos;s most pristine wilderness, where penguins rule and glaciers tell ancient stories.</p>
        </div>
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {antarcticaDestinations.map((dest) => (
            <div key={dest.name} className="flex flex-col rounded-lg bg-card p-6 shadow-md transition-shadow hover:shadow-xl">
              <h2 className="mb-2 text-2xl font-bold">{dest.name}</h2>
